...
@app.route("/login")
def login():
    return render_template("login.html")@app.route("/forgot-password")
def forgot_password():
...
def api_game_images():
    from game.game_logic import get_game_images
...
def api_guess():
    ...
    score, distance, actual_lat, actual_lng = calculate_score(
        guess_lat, guess_lng, img_id
    )
    if score is None:
